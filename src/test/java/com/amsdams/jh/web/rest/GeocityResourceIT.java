...
public class GeocityResourceIT {
	...
	private static final Point DEFAULT_LOCATION = createPoint("42", "5");
	private static final Point UPDATED_LOCATION = createPoint("44", "5");
	...
	/**
	 * This repository is mocked in the com.amsdams.jh.repository.search test
	 * package.
	 *
	...
	private static Point createPoint(String lat, String lon) {
		Position g2d = new G2D(Double.valueOf(lat), Double.valueOf(lon));
	...
	public static Geocity createEntity(EntityManager em) {
		Geocity geocity = new Geocity().name(DEFAULT_NAME).asciiname(DEFAULT_ASCIINAME)
				.alternatenames(DEFAULT_ALTERNATENAMES).location(DEFAULT_LOCATION).lat(DEFAULT_LAT).lon(DEFAULT_LON)
				.featureclass(DEFAULT_FEATURECLASS).featuretype(DEFAULT_FEATURETYPE).countrycode(DEFAULT_COUNTRYCODE)
				.cc2(DEFAULT_CC_2).admin1code(DEFAULT_ADMIN_1_CODE).admin2code(DEFAULT_ADMIN_2_CODE)
				.admin3code(DEFAULT_ADMIN_3_CODE).admin4code(DEFAULT_ADMIN_4_CODE).population(DEFAULT_POPULATION)
				.elevation(DEFAULT_ELEVATION).dem(DEFAULT_DEM);
		return geocity;
	}
	...
	public static Geocity createUpdatedEntity(EntityManager em) {
		Geocity geocity = new Geocity().name(UPDATED_NAME).asciiname(UPDATED_ASCIINAME)
				.alternatenames(UPDATED_ALTERNATENAMES).location(UPDATED_LOCATION).lat(UPDATED_LAT).lon(UPDATED_LON)
				.featureclass(UPDATED_FEATURECLASS).featuretype(UPDATED_FEATURETYPE).countrycode(UPDATED_COUNTRYCODE)
				.cc2(UPDATED_CC_2).admin1code(UPDATED_ADMIN_1_CODE).admin2code(UPDATED_ADMIN_2_CODE)
				.admin3code(UPDATED_ADMIN_3_CODE).admin4code(UPDATED_ADMIN_4_CODE).population(UPDATED_POPULATION)
				.elevation(UPDATED_ELEVATION).dem(UPDATED_DEM);
		return geocity;
	}
	...
	public void createGeocity() throws Exception {
		...
		restGeocityMockMvc.perform(post("/api/geocities").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(geocityDTO))).andExpect(status().isCreated());
	...
	public void createGeocityWithExistingId() throws Exception {
		...
		restGeocityMockMvc.perform(post("/api/geocities").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(geocityDTO))).andExpect(status().isBadRequest());
	...
	public void getAllGeocities() throws Exception {
		...
		restGeocityMockMvc.perform(get("/api/geocities?sort=id,desc")).andExpect(status().isOk())
	...
	public void getGeocity() throws Exception {
		...
		// Get the geocity
		restGeocityMockMvc.perform(get("/api/geocities/{id}", geocity.getId())).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
	...
	public void getNonExistingGeocity() throws Exception {
		// Get the geocity
		restGeocityMockMvc.perform(get("/api/geocities/{id}", Long.MAX_VALUE)).andExpect(status().isNotFound());
	}
	...
	public void updateGeocity() throws Exception {
		...
		// Disconnect from session so that the updates on updatedGeocity are not
		// directly saved in db
		em.detach(updatedGeocity);
		updatedGeocity.name(UPDATED_NAME).asciiname(UPDATED_ASCIINAME).alternatenames(UPDATED_ALTERNATENAMES)
				.location(UPDATED_LOCATION).lat(UPDATED_LAT).lon(UPDATED_LON).featureclass(UPDATED_FEATURECLASS)
				.featuretype(UPDATED_FEATURETYPE).countrycode(UPDATED_COUNTRYCODE).cc2(UPDATED_CC_2)
				.admin1code(UPDATED_ADMIN_1_CODE).admin2code(UPDATED_ADMIN_2_CODE).admin3code(UPDATED_ADMIN_3_CODE)
				.admin4code(UPDATED_ADMIN_4_CODE).population(UPDATED_POPULATION).elevation(UPDATED_ELEVATION)
				.dem(UPDATED_DEM);
		GeocityDTO geocityDTO = geocityMapper.toDto(updatedGeocity);

		restGeocityMockMvc.perform(put("/api/geocities").contentType(TestUtil.APPLICATION_JSON_UTF8)
				.content(TestUtil.convertObjectToJsonBytes(geocityDTO))).andExpect(status().isOk());
	...
	public void deleteGeocity() throws Exception {
		...
		// Delete the geocity
		restGeocityMockMvc
				.perform(delete("/api/geocities/{id}", geocity.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
	...
	public void searchGeocity() throws Exception {
		...
		// Search the geocity
		restGeocityMockMvc.perform(get("/api/_search/geocities?query=id:" + geocity.getId())).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
				.andExpect(jsonPath("$.[*].id").value(hasItem(geocity.getId().intValue())))
				.andExpect(jsonPath("$.[*].name").value(hasItem(DEFAULT_NAME)))
				.andExpect(jsonPath("$.[*].asciiname").value(hasItem(DEFAULT_ASCIINAME)))
